...
# ________________________________________________________________________----
# READ Q2K OUTPUT
read_q2k_out <- function(mOut = NULL) { #}, pars = NULL) {

  # Function that reads QUAL2Kw output files (full) and returns a data frame
  # of timeseries of results of specified parameters
...
add_met <- function(scen = NULL, strD = NULL, endD = NULL, seas = NULL) {

  suppressMessages(library(reshape2))

  mtPr <- c('airT', 'cCov', 'dwpT', 'wndS')

  # Specify the met files
  mtFl <- paste0('C:/siletz_tmdl/01_inputs/02_q2k/',
                 paste0('mt_', c('Ta', 'CC', 'Td', 'wnd')), '/', scen, '_',
                 mtPr, '_', seas, '.csv')

  # Loop through each file, read data and process into output data frame
  for (i in 1 : length(mtFl)) {
