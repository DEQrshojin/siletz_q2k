...
# write.csv(file = 'C:/siletz_tmdl/02_outputs/02_q2k/load_cap/rvr_loads_R07_YR06.csv',
#           x = cOut[['B13']], row.names = F)
...
# HARDWIRE CODE TO ADJUST WATERSHED MODEL INPUTS FOR Q2K STABILITY:
# if (year(strD) == 2013 | year(strD) == 2016) {
#   
#   cOut$HW$qIn_cms <- cOut$HW$qIn_cms * 0.95
#   
#   for (j in 2 : 11) {
#     
#     cOut[[j]]$abs_cms <- cOut[[j]]$abs_cms * 0.50
#     
#     cOut[[j]]$oss_mgL <- cOut[[j]]$oss_mgL * 0.90
#       
#   }
# }
# HARDWIRE CODE TO ADJUST WATERSHED MODEL INPUTS FOR Q2K STABILITY:
